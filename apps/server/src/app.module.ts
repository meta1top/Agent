...
export class AppModule {
  static forRoot(preloadedConfig: AppConfig | null): DynamicModule {
    ...
    const imports: DynamicModule["imports"] = [
      ...
      CommonModule,
      SharedModule.forRoot(preloadedConfig!),
    ];
    ...
    if (preloadedConfig?.database) {
      ...
      const typeOrmConfig = {
        ...
        migrationsRun: true,
        ...databaseConfig,
        ...(databaseConfig.logging && !isDevelopment && { logger: new PlainTextLogger() }),
      } as Parameters<typeof TypeOrmModule.forRoot>[0];

      imports.push(TypeOrmModule.forRoot(typeOrmConfig));
    } else {
    ...
    if (preloadedConfig?.redis) {
      ...
      imports.push({
        ...BullModule.registerQueue({
          name: APP_TASK_QUEUE_NAME,
          connection: {
            ...preloadedConfig.redis.options,
          },
          defaultJobOptions: preloadedConfig?.taskQueue ?? DEFAULT_TASK_QUEUE_CONFIG,
        }),
        global: true,
      });
    } else {
